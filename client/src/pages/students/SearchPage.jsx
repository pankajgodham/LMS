...
const SearchPage = () => {
    ...
    const isEmpty=false
  return (
    <div className='max-w-7xl mx-auto mt-5 p-4 md:p-8'>
        <div className='my-6'>
            <h1>
result for "html"
            </h1>
            ...
            <div className='flex-1'>
                {
                    isLoading?(
                        Array.from({length:3}).map((_,idx)=>(
                            <CourceSkeleton key={idx}/>
                        ))
                    ):isEmpty?(<CourceNotFound/>):(
                        [1,2,3].map((cource,idx)=><SearchResult key={idx}/>)
                    )
                }
            </div>
        </div>
